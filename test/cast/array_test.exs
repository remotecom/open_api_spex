defmodule OpenApiSpec.Cast.ArrayTest do
  ...
  describe "cast/4" do
    ...
    test "with max items" do
      schema = %Schema{type: :array, items: %Schema.Items{max: 2}}
    ...
    test "with min items" do
      schema = %Schema{type: :array, items: %Schema.Items{min: 2}}
      assert cast(value: [1, 2], schema: schema) == {:ok, [1, 2]}
      ...
      assert cast(value: [], schema: %Schema{type: :array, items: %Schema.Items{min: 0}}) ==
               {:ok, []}
    end

    test "with unique items" do
      schema = %Schema{type: :array, items: %Schema.Items{unique: true}}
      assert cast(value: [1, 2], schema: schema) == {:ok, [1, 2]}
    ...
    test "array with items config" do
      items_schema = %Schema{type: :integer}
      schema = %Schema{type: :array, items: %Schema.Items{value: items_schema}}
      assert cast(value: [], schema: schema) == {:ok, []}
      assert cast(value: [1, 2, 3], schema: schema) == {:ok, [1, 2, 3]}
      assert cast(value: ["1", "2", "3"], schema: schema) == {:ok, [1, 2, 3]}
